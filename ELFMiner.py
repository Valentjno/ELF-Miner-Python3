...
def symbols_table(file):
    ...
    syms = subprocess.check_output(["readelf", "-s", file], text=True)
...
def relocation_section(file):
    ...
    reloc = subprocess.check_output(["readelf", "-r", file], text=True)
